...
export function useIncome() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, 
      ...
      (err) => {
        console.error('Error listening to income:', err);
        setError(err.message);
        setLoading(false);
      }
    );
  ...
  const updateIncome = async (id: string, updatedData: Partial<Omit<Income, 'id' | 'userId' | 'createdAt'>>) => {
    if (!user) return;
  ...
  return { income, loading, error, addIncome, updateIncome, deleteIncome };
}
...
export const usePortfolioSummary = () => {
  ...
  }, [user, income, expenses, assets, liabilities]);
...
// -------------------- MONTHLY INCOME --------------------
export function useMonthlyIncome(monthIndex?: number) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, 
      ...
      (err) => {
        console.error('Error listening to monthly income:', err);
        setError(err.message);
        setLoading(false);
      }
    );
